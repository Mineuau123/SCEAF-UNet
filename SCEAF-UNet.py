...
class SCEAF_UNet(nn.Module):
    def __init__(self, in_channels=1, num_classes=1, img_size=224, pretrained_path='net_B.pth'):
        ...
        self.eaf3 = EAF(self.embed_dims[2])
        self.eaf2 = EAF(self.embed_dims[1])
        self.eaf1 = EAF(self.embed_dims[0])

        self.sceaf_attention_dec2 = SCEAF(in_channels=self.embed_dims[1], out_channels=self.embed_dims[1])
        self.sceaf_attention_dec3 = SCEAF(in_channels=self.embed_dims[0], out_channels=self.embed_dims[0])

        self.decoder1 = CSU(self.embed_dims[3], self.embed_dims[2])
        self.decoder2 = CSU(self.embed_dims[2]*2, self.embed_dims[1])
        self.decoder3 = CSU(self.embed_dims[1]*2, self.embed_dims[0])
        self.decoder4 = CSU(self.embed_dims[0]*2, 24)
        self.final_conv = nn.Conv2d(24, num_classes, kernel_size=1)

    def forward(self, x):
        # Encoder
        x = x.repeat(1, 3, 1, 1)  
        for blk in self.encoder.stage0: x = blk(x)
        enc0 = x
        for blk in self.encoder.stage1: x = blk(x)
        enc1 = x
        for blk in self.encoder.stage2: x = blk(x)
        enc2 = x
        for blk in self.encoder.stage3: x = blk(x)
        enc3 = x
        for blk in self.encoder.stage4: x = blk(x)
        ...
        dec2 = self.sceaf_attention_dec2(enc2, dec2) 
        dec1 = self.decoder3(torch.cat([dec2, enc2], dim=1))
        dec1 = self.sceaf_attention_dec3(enc1, dec1)  
